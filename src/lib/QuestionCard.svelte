...
<div class="card bg-blue-500 text-white rounded p-4 mb-4">
  ...
  <ul class="answers flex flex-wrap gap-4">
    {#each answers as answer }
      <button class="answer w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5 bg-white text-black border border-gray-300 p-2 rounded cursor-pointer" on:click={() => handleAnswerClick(answer)} id={`answer-${answer.id_risposta}`}>
